...
import { createPersistedQueryLink as createPersistedQuery, VERSION } from "apollo-link-persisted-queries";
import { createApolloFetch, ApolloFetch, GraphQLRequest, ParsedResponse } from "apollo-fetch";
import {
  AuthenticationError,
  UserInputError,
  gql,
...
export function createServerInfo<AS extends ApolloServerBase>(server: AS, httpServer: http.Server): ServerInfo<AS> {
...
const makeGatewayMock = ({
  ...
  const eventuallyAssigned = {
    resolveLoad: null as unknown as ({ schema, executor }) => void,
    rejectLoad: null as unknown as (err: Error) => void,
    triggerSchemaChange: null as unknown as (newSchema) => void
  };
...
export function testApolloServer<AS extends ApolloServerBase>(
  ...
  describe(`ApolloServer`, () => {
    ...
    describe(`constructor`, () => {
      describe(`validation rules`, () => {
        it(`accepts additional rules`, async () => {
          const NoTestString = (context: ValidationContext) => ({
            Field(node: FieldDefinitionNode) {
              if (node.name.value === `testString`) {
                context.reportError(new GraphQLError(`Not allowed to use`, [node]));
              }
            }
          });
          ...
          expect(introspectionResult.errors[0].message).toMatch(/introspection/);
          expect(formatError.mock.calls).toHaveLength(introspectionResult.errors.length);

          const result = await apolloFetch({ query: TEST_STRING_QUERY });
          expect(result.data).toBeUndefined();
          expect(result.errors).toBeDefined();
          expect(result.errors[0].message).toMatch(/Not allowed/);
          expect(formatError.mock.calls).toHaveLength(introspectionResult.errors.length + result.errors.length);
        });
        ...
        it(`prevents introspection by default during production`, async () => {
          ...
          expect(result.errors[0].extensions.code).toEqual(`GRAPHQL_VALIDATION_FAILED`);
        ...
        it(`prohibits providing a gateway in addition to schema/typedefs/resolvers`, async () => {
          ...
          await createApolloServer({ gateway, schema }).catch(err => incompatibleArgsSpy(err.message));
          expect(incompatibleArgsSpy.mock.calls[0][0]).toMatch(/Cannot define both/);

          await createApolloServer({ gateway, modules: {} as any }).catch(err => incompatibleArgsSpy(err.message));
          expect(incompatibleArgsSpy.mock.calls[1][0]).toMatch(/Cannot define both/);

          await createApolloServer({ gateway, typeDefs: {} as any }).catch(err => incompatibleArgsSpy(err.message));
          expect(incompatibleArgsSpy.mock.calls[2][0]).toMatch(/Cannot define both/);
        });

        it(`prohibits providing a gateway in addition to subscription options`, async () => {
          const { gateway } = makeGatewayMock();

          const expectedError = `Subscriptions are not yet compatible with the gateway`;
      ...
      describe(`schema creation`, () => {
        ...
        it(`accepts a gateway's schema and calls its executor`, async () => {
          ...
          executor.mockReturnValue(Promise.resolve({ data: { testString: `hi - but federated!` } }));
        ...
        it(`rejected load promise acts as an error boundary`, async () => {
          const executor = jest.fn();
          executor.mockResolvedValueOnce({ data: { testString: `should not get this` } });

          executor.mockRejectedValueOnce({ errors: [{ errorWhichShouldNot: `ever be triggered` }] });
          ...
          expect(consoleErrorSpy).toHaveBeenCalledWith(
            `This data graph is missing a valid configuration. ` + `load error which should be masked`
          );
    ...
    describe(`Plugins`, () => {
      ...
      const setupApolloServerAndFetchPairForPlugins = async (plugins: PluginDefinition[] = []) => {
      ...
      it(`works when using executeOperation`, async () => {
        const encounteredFields: any = [];
        const encounteredContext: any = [];
        ...
        expect(encounteredFields).toStrictEqual([
          { key: `justAField`, prev: undefined }, // eslint-disable-line
          { key: `justAField`, prev: undefined } // eslint-disable-line
        ]);
      ...
      it(`preserves user-added "extensions" in the response when validation errors occur`, async () => {
        ...
        const result = await apolloFetch({ query: `{ missingFieldWhichWillNotValidate }` });
        expect(result.errors).toBeDefined();
        expect(result.extensions).toEqual(
          expect.objectContaining({
            myExtension: true
    ...
    describe(`formatError`, () => {
      ...
      it(`works with errors similar to GraphQL errors, such as yup`, async () => {
        ...
        const throwError = jest.fn(async () => {
          ...
          yuppieError.type = undefined; // eslint-disable-line
          ...
          yuppieError.params = {
            ...
            label: undefined, // eslint-disable-line
            regex: /@/
          };
        ...
        const { url: uri } = await createApolloServer({
          ...
          resolvers: {
            Query: {
              fieldWhichWillError: () => throwError()
            }
          },
          introspection: true,
          debug: true,
    ...
    describe(`lifecycle`, () => {
      describe(`for Apollo Graph Manager`, () => {
        ...
        describe(`extensions`, () => {
          ...
          it(`calls formatError and other overloaded client identity tests`, async () => {
            ...
            class Extension<TContext = any> extends GraphQLExtension {
              willSendResponse(o: { graphqlResponse: GraphQLResponse; context: TContext }) {
                expect(o.graphqlResponse.errors).toHaveLength(1);
        ...
        describe(`traces`, () => {
          ...
          it(`does not expose stack`, async () => {
            ...
            expect(trace.root.child).toHaveLength(1);

            // The error should not have the stack in it.
            expect(trace.root.child[0].error[0]).not.toHaveProperty(`stack`);
            expect(JSON.parse(trace.root.child[0].error[0].json)).not.toHaveProperty(`stack`);
          });
          ...
          it(`sets the trace key to operationName when it is defined`, async () => {
            ...
            expect(Object.keys(reports[0].tracesPerQuery)[0]).toMatch(/^# AnOperationName\n/);
          });
          ...
          it(`doesn't internal server error on an APQ`, async () => {
            ...
            const TEST_STRING_QUERY = `
              { onlyForThisApqTest${Math.random().toString().split(`.`)[1]}: justAField }
            `;
          ...
          describe(`error munging`, () => {
            describe(`rewriteError`, () => {
              it(`new error`, async () => {
                ...
                await setupApolloServerAndFetchPair({
                  rewriteError: () => new GraphQLError(`rewritten as a new error`)
                });
              ...
              it(`nulled error`, async () => {
                ...
                expect(result.errors).toBeDefined();
                expect(result.errors[0].message).toEqual(`rewriteError null nope`);
                expect(throwError).toHaveBeenCalledTimes(1);
            ...
            it(`undefined error`, async () => {
              ...
              expect(result.errors).toBeDefined();
              expect(result.errors[0].message).toEqual(`rewriteError undefined whoops`);
              expect(throwError).toHaveBeenCalledTimes(1);
            ...
            it(`maskErrorDetails (legacy)`, async () => {
              ...
              expect(trace.root.child[0].error).toMatchObject([
                {
                  json: `{"message":"<masked>","locations":[{"line":1,"column":2}],"path":["fieldWhichWillError"]}`,
                  message: `<masked>`,
      ...
      it(`errors thrown in extensions call formatError and are wrapped`, async () => {
        ...
        class Extension<TContext = any> extends GraphQLExtension {
          willSendResponse(_o: { graphqlResponse: GraphQLResponse; context: TContext }) {
    ...
    describe(`subscriptions`, () => {
      ...
      `;
        ...
        createApolloServer({
          ...
          const client = new SubscriptionClient(`ws://localhost:${port}${server.subscriptionsPath}`, {}, WebSocket);
      ...
      `;
        ...
        createApolloServer({
          ...
          try {
            server.installSubscriptionHandlers(httpServer);
            done.fail(`subscription server creation should fail, since subscriptions are disabled`);
          } catch (e) {
    ...
    describe(`Persisted Queries`, () => {
      ...
      it(`returns correct result for persisted query link`, done => {
        ...
        const link = createPersistedQuery().concat(createHttpLink({ uri, fetch } as any));
    ...
    describe(`apollo-engine-reporting`, () => {
      async function makeFakeTestableEngineServer({
        ...
        const writeResponsePromise = new Promise(resolve => (writeResponseResolve = resolve));
      ...
      describe(`graphql server functions even when Apollo servers are down`, () => {
        async function testWithStatus(status: number, expectedRequestCount: number) {
          const networkError = status === 0;

          const { closeServer, fakeEngineUrl, writeResponseResolve } = await makeFakeTestableEngineServer({
            status,
          ...
          try {
            ...
            const reportErrorPromise = new Promise<Error>(resolve => (reportErrorPromiseResolve = resolve));
            const { url: uri } = await createApolloServer({
              typeDefs: gql`
                type Query {
                  something: String!
            ...
            if (networkError) {
              expect(sendingError.message).toContain(`Error sending report to Apollo Engine servers`);
    ...
    describe(`Tracing`, () => {
      ...
      const resolvers = {
        Query: {
          books: () => new Promise(resolve => setTimeout(() => resolve([{ title: `H`, author: `J` }]), 10)),
          movies: () => new Promise(resolve => setTimeout(() => resolve([{ title: `H` }]), 12))
        }
      };
      ...
      it(`reports a total duration that is longer than the duration of its resolvers`, async () => {
        ...
        const earliestStartOffset = tracing.execution.resolvers
          ...
          .reduce((currentEarliestOffset, nextOffset) => Math.min(currentEarliestOffset, nextOffset));

        const latestEndOffset = tracing.execution.resolvers
          .map(resolver => resolver.startOffset + resolver.duration)
          .reduce((currentLatestEndOffset, nextEndOffset) => Math.max(currentLatestEndOffset, nextEndOffset));
    ...
    describe(`Federated tracing`, () => {
      ...
      it(`includes errors in federated trace`, async () => {
        ...
        const encoded = Buffer.from(ftv1, `base64`);
        const trace = Trace.decode(encoded);
        expect(trace.root.child[0].error[0].message).toBe(`Rewritten for Engine: It broke`);
      });
    });
    ...
    describe(`Response caching`, () => {
      ...
      it(`basic caching`, async () => {
        ...
        const expectedResolverCallCount: Partial<Record<FieldName, number>> = {};
        const expectCacheHit = (fn: FieldName) => expect(resolverCallCount[fn]).toBe(expectedResolverCallCount[fn]);
        const expectCacheMiss = (fn: FieldName) => expect(resolverCallCount[fn]).toBe(++expectedResolverCallCount[fn]);
          ...
          plugins: [
            ApolloServerPluginResponseCache({
              ...
              extraCacheKeyData: (requestContext: GraphQLRequestContext<any>) =>
                requestContext.request.http.headers.get(`extra-cache-key-data`) || null,
        ...
        // Just for the typing.
        function doFetch(options: GraphQLRequest & { headers?: Record<string, string> }) {
        ...
        // Cache miss
        {
          ...
          expect(httpHeader(result, `age`)).toBeNull();
        }
        ...
        // full response cache).
        {
          ...
          expect(httpHeader(result, `cache-control`)).toBeNull();
          expect(httpHeader(result, `age`)).toBeNull();
        }
    ...
    describe(`Gateway`, () => {
      ...
      it(`passes engine data to the gateway`, async () => {
        ...
        expect(optionsSpy).toHaveBeenLastCalledWith({
          engine: {
            apiKeyHash: `0ca858e7fe8cffc01c5f1db917d2463b348b50d267427e54c1c8c99e557b242f4145930b949905ec430642467613610e471c40bb7a251b1e2248c399bb0498c4`,
            graphId: `tester`,
      ...
      it(`can serve multiple active schemas simultaneously during a schema rollover`, async () => {
        ...
        const makeEventuallyResolvingPromise = val => {
          let resolver;
          const promise = new Promise(resolve => (resolver = () => resolve(val)));
        ...
        const executor = req => ((req.source as string).match(/1/) ? p1 : (req.source as string).match(/2/) ? p2 : p3);
