...
import { GraphQLOptions, Config, PersistedQueryOptions, KeyValueCache } from "apollo-server-core";
...
export const NODE_MAJOR_VERSION: number = parseInt(process.versions.node.split(`.`, 1)[0], 10);
...
export interface CreateAppOptions {
  ...
  graphqlOptions?: GraphQLOptions | { (): ValueOrPromise<GraphQLOptions> } | Config;
}
  ...
  describe(`apolloServer`, () => {
    ...
    describe(`graphqlHTTP`, () => {
      ...
      it(`can handle a basic request with cacheControl`, async () => {
        app = await createApp({ schema, cacheControl: true });
        const expected = {
      ...
      it(`returns PersistedQueryNotSupported to a GET request if PQs disabled`, async () => {
        app = await createApp({ schema, persistedQueries: false });
        ...
        return req.then(res => {
          ...
          expect(res.body.errors[0].message).toEqual(`PersistedQueryNotSupported`);
        });
      });
      ...
      it(`can handle introspection request`, async () => {
        app = await createApp();
        const req = request(app).post(`/graphql`).send({ query: getIntrospectionQuery() });
        return req.then(res => {
          expect(res.status).toEqual(200);
          expect(res.body.data.__schema.types[0].fields[0].name).toEqual(`testString`);
        });
      });
      ...
      describe(`lifecycle hooks`, () => {
        it(`calls serverWillStart before serving a request`, async () => {
          ...
          const delayUntil = async (check: () => boolean, expectedNumTicks) => {
            ...
            expect(expectedNumTicks).not.toBe(0);
            return new Promise(resolve =>
              process.nextTick(() => delayUntil(check, expectedNumTicks - 1).then(resolve))
            );
          };
